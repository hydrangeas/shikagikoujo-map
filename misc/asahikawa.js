...
var inputFilePath = 'asahikawa-input.csv';
var outputFilePath = 'asahikawa.csv';
    ...
    return __generator(this, function (_b) {
        switch (_b.label) {
            ...
            case 1:
                ...
                address = "".concat(facility.市町村).concat(facility.所在地).concat(facility.所在地２);
                return [4 /*yield*/, fetch("".concat(apiUrl, "?key=").concat(apiKey, "&address=").concat(encodeURIComponent("\u3012".concat(facility.郵便番号, " \u5317\u6D77\u9053").concat(facility.市町村).concat(facility.所在地))))];
            case 2:
                response = _b.sent();
            ...
            case 3:
                ...
                for (i = 1; i < data.results.length; i++) {
                    console.log("0 ".concat(facility.名称, " (").concat(address, "), ").concat(location_1.lat, ", ").concat(location_1.lng, ",").concat(data.results[0].geometry.location_type));
                    result = data.results[i];
                    console.log("".concat(i, " ").concat(facility.名称, " (").concat(address, "), ").concat(result.geometry.location.lat, ", ").concat(result.geometry.location.lng, ", ").concat(result.geometry.location_type));
