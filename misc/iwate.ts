...
const inputFilePath = 'iwate-input.csv';
const outputFilePath = 'iwate.csv';
...
fs.createReadStream(inputFilePath)
  ...
  .on('end', async () => {
    ...
    for (const facility of facilities) {
      const address = `岩手県${facility.所在地}`;
    ...
    console.log('iwate.csv has been written successfully.');
  });
// npx tsc iwate.ts && node iwate.js | tee iwate.log